...
const problems = [
  {
    icon: '家',
    title: ['自分の家が', 'いくらで売れるか知りたい'],
    text: 'まずは相場だけ知りたい段階でも大丈夫です。'
  },
  {
    icon: '相',
    title: ['相続不動産の扱いに', '迷っている'],
    text: '売却・賃貸・保有を比較して整理します。'
  },
  {
    icon: '住',
    title: ['住み替え時期が', '不安'],
    text: '購入と売却の順番、資金計画まで確認します。'
  },
  {
    icon: '空',
    title: ['空き家の管理・売却で', '困っている'],
    text: '放置リスクや売却時期も含めてご案内します。'
  }
];

const reasons = [
  [['大阪市専門の', '地域密着サポート'], '近隣事例・買主動向・販売状況を踏まえ、現実的な売却方針をご提案します。'],
  [['しつこい営業をしない', '誠実な対応'], '売ることを急がせず、売主様のペースで判断できるよう丁寧に整理します。'],
  [['相続・住み替え・', '空き家に強い'], '価格だけでなく、ご家族・ローン・今後の暮らしまで一緒に考えます。'],
  [['売る・貸す・保有を', '比較して提案'], '売却が正解とは限りません。複数の選択肢を比較して判断を支えます。'],
  [['契約から引き渡しまで', '丁寧にサポート'], '販売活動から条件交渉、契約、お引き渡しまで一貫して伴走します。']
];
  ...
  return (
    <main>
        ...
        <div className="container heroGrid">
          <div className="heroCopy">
            ...
            <h1 className="heroTitle" aria-label="売るか迷っている。それなら、まずは相談してください。">
              <span>売るか、</span>
              <span>迷っている。</span>
              <span className="goldText heroGoldTop">それなら、</span>
              <span className="goldText heroGoldMain">まずは</span>
              <span className="goldText">相談してください。</span>
            </h1>
      ...
      <section className="section dark" id="problem">
        <div className="container">
          ...
          <div className="problemGrid">
            {problems.map((item) => (
              <div className="problemCard" key={item.title.join('')}>
                <div className="iconCircle">{item.icon}</div>
                <h3>{item.title.map((line) => <span key={line}>{line}</span>)}</h3>
                <p>{item.text}</p>
              </div>
            ))}
          </div>
        ...
        <div className="container">
          ...
          <div className="reasonGrid">
            {reasons.map(([title, text], i) => (
              <article className="reasonCard" key={Array.isArray(title) ? title.join('') : title}>
                <div className="reasonNo">0{i + 1}</div>
                <h3>{Array.isArray(title) ? title.map((line) => <span key={line}>{line}</span>) : title}</h3>
                <p>{text}</p>
              </article>
            ))}
          </div>
      ...
      <section className="section" id="blog">
        <div className="container">
          <SectionTitle eyebrow="BLOG / REPORT" title="売却レポート・お役立ちブログ" />
...
function SocialCard({ url, icon, title, sub }) {
  return <a className="snsCard" href={url} target="_blank" rel="noreferrer"><img src={icon} alt={title} /><span><strong>{title}</strong><small>{sub}</small></span></a>
}

function SectionTitle({ eyebrow, title, text, dark = false }) {
  return <div className={`sectionTitle ${dark ? 'titleDark' : ''}`}><span className="sectionEyebrow">{eyebrow}</span><h2>{title}</h2>{text && <p>{text}</p>}</div>
}

function Footer() {
  return <footer className="footer"><div className="container footerGrid"><div><h3>北村　充</h3><p>大阪市の不動産売却相談</p></div><div><h2>090-7555-6542</h2><p>株式会社ランド 阿波座店<br />大阪市西区立売堀5-4-1</p></div><div><p>SNS</p><div className="footerSns"><a href={INSTAGRAM_URL} target="_blank" rel="noreferrer"><img src="/icons/instagram.png" alt="Instagram" /></a><a href={TIKTOK_URL} target="_blank" rel="noreferrer"><img src="/icons/tiktok.png" alt="TikTok" /></a><a href={LINE_URL} target="_blank" rel="noreferrer"><img src="/icons/line.png" alt="LINE" /></a></div></div></div><div className="container copy">© 2026 北村　充 不動産売却サポート</div></footer>
}
