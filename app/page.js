...
import { blogs as guideBlogs } from './data/blogs';
...
export default function Home() {
  return (
    <main>
      ...
      <section className="section" id="blog"><div className="container"><SectionTitle eyebrow="BLOG / REPORT" title="売却レポート・お役立ちブログ" /><div className="blogGrid">{guideBlogs.slice(0,3).map((blog) => <article className="blogCard" key={blog.slug}><small>{blog.category}</small><h3>{blog.title}</h3><p>{blog.description}</p><Link href={`/blog/${blog.slug}`}>続きを読む</Link></article>)}</div><div className="center"><Link className="btn btnPhone" href="/blog">売却ガイドをもっと見る</Link></div></div></section>
...
function Header(){return <header className="header"><div className="container nav"><a className="brand" href="#top"><span className="brandMark">北</span><span><strong>北村　充</strong><small>大阪市の不動産売却相談</small></span></a><nav><a href="#problem">お悩み</a><a href="#reason">選ばれる理由</a><a href="#ai">AI査定</a><a href="#about">北村について</a><a href="/blog">ブログ</a><a href="#area">対応エリア</a><a href="#faq">FAQ</a></nav><div className="headActions"><a className="headSocial" href={INSTAGRAM_URL} target="_blank" rel="noreferrer"><img src="/icons/instagram.png" alt="Instagram" /></a><a className="headSocial" href={TIKTOK_URL} target="_blank" rel="noreferrer"><img src="/icons/tiktok.png" alt="TikTok" /></a><a className="headSocial" href={LINE_URL} target="_blank" rel="noreferrer"><img src="/icons/line.png" alt="LINE" /></a><a className="phonePill" href={`tel:${PHONE}`}>{PHONE}</a></div></div></header>}
