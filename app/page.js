...
const problems = [
  { icon: '家', title: '家がいくらで売れるか知りたい', text: '売る前の段階から、現実的な選択肢を整理します。' },
  { icon: '相', title: '相続不動産の進め方がわからない', text: '売却・賃貸・保有を比較して、進め方を一緒に考えます。' },
  { icon: '住', title: '住み替えのタイミングが不安', text: '購入と売却の順番、資金計画まで確認します。' },
  { icon: '空', title: '空き家の管理・売却に困っている', text: '放置リスクや売却時期も含めてご案内します。' }
];

const reasons = [
  { no: '01', title: '大阪市専門の地域密着', text: '近隣事例・買主動向・販売状況を踏まえ、現実的な売却方針をご提案します。' },
  { no: '02', title: 'しつこい営業なし', text: '売ることを急がせず、売主様のペースで判断できるよう丁寧に整理します。' },
  { no: '03', title: '相続・住み替え・空き家', text: '価格だけでなく、ご家族・ローン・今後の暮らしまで一緒に考えます。' },
  { no: '04', title: '売る・貸す・保有を比較', text: '売却が正解とは限りません。複数の選択肢を比較して判断を支えます。' },
  { no: '05', title: '契約から引渡しまで伴走', text: '販売活動から条件交渉、契約、お引き渡しまで一貫して伴走します。' }
];
    ...
    <main>
        ...
        <div className="container heroGrid">
          <div className="heroCopy">
            ...
            <h1 className="heroTitle">
              ...
              <span className="goldText">まずは相談</span>
              <span className="goldText">してください。</span>
            </h1>
            <p className="lead">今すぐ売る予定がなくても大丈夫です。相場を知り、売る・貸す・保有する選択肢を一緒に整理します。</p>
            <div className="ctaRow">
              <a className="btn btnLine" href={LINE_URL} target="_blank" rel="noreferrer"><img src="/icons/line.png" alt="" />LINEで無料相談する</a>
              <a className="btn btnAi" href={AI_URL} target="_blank" rel="noreferrer">AIで相場を調べる</a>
              <a className="btn btnPhone" href={`tel:${PHONE}`}>電話で相談する</a>
            </div>
          </div>
          <div className="heroPhoto">
            <img src="/images/hero-phone.jpg" alt="電話で相談対応をする北村　充" />
          </div>
        </div>
      </section>
      ...
      <section className="section dark" id="problem">
        <div className="container">
          ...
          <div className="problemGrid">
            {problems.map((item) => (
              <article className="problemCard" key={item.icon}>
      ...
      <section className="section" id="reason">
        <div className="container">
          ...
          <div className="reasonGrid">
            {reasons.map((item) => (
              <article className="reasonCard" key={item.no}>
                <div className="reasonNo">{item.no}</div>
                <h3>{item.title}</h3>
                <p>{item.text}</p>
              </article>
            ))}
      ...
      <section className="section dark" id="voice">
        <div className="container">
          ...
          <div className="voiceGrid">
            {voices.map(([area, title, text]) => (
              <article className="voiceCard" key={title}><small>{area}</small><h3>{title}</h3><p>{text}</p></article>
            ))}
          </div>
        </div>
      </section>
      ...
      <section className="section aiSection" id="ai">
        <div className="container aiGrid">
          ...
          <div className="aiCompare">
            <h3>AI査定と北村の査定の違い</h3>
            <p><strong>AI査定：</strong>まず相場の目安を知る。</p>
            <p><strong>北村の査定：</strong>販売戦略、売却時期、住み替え、相続、税金面まで整理する。</p>
            <a className="btn btnLine" href={LINE_URL} target="_blank" rel="noreferrer"><img src="/icons/line.png" alt="" />査定結果の見方を相談する</a>
          </div>
        </div>
      </section>
      ...
      <section className="section" id="flow">
        <div className="container">
          ...
          <div className="flowGrid">
            {flow.map(([no, title, text]) => <article className="flowCard" key={no}><span>{no}</span><h3>{title}</h3><p>{text}</p></article>)}
          </div>
        </div>
      </section>
      ...
      <section className="section" id="blog">
        <div className="container">
          <SectionTitle eyebrow="BLOG / REPORT" title="売却ガイド・ブログ" />
          <div className="blogGrid">
            {blogs.map((item) => <article className="blogCard" key={item.title}><h3>{item.title}</h3><p>{item.text}</p><a className="textLink" href={item.href}>記事を読む</a></article>)}
          </div>
          <div className="center"><a className="btn btnAi" href="/blog">売却ガイド一覧を見る</a></div>
        </div>
      </section>

      <section className="section" id="area">
        <div className="container"><SectionTitle eyebrow="AREA" title="対応エリア" text="大阪市全域に対応しています。特に下記エリアの売却相談を強化しています。" /><div className="areaTags">{areas.map(a => <span key={a}>{a}</span>)}</div></div>
      </section>
...
function Header(){return <header className="header"><div className="container nav"><a className="brand" href="/#top"><span className="brandMark">北</span><span><strong>北村　充</strong><small>大阪市の不動産売却相談</small></span></a><nav><a href="/#problem">お悩み</a><a href="/#reason">選ばれる理由</a><a href="/#flow">売却の流れ</a><a href="/#ai">AI査定</a><a href="/blog">ブログ</a><a href="/#about">北村について</a><a href="/#area">対応エリア</a><a href="/#faq">FAQ</a></nav><div className="headActions"><a className="headSocial" href={INSTAGRAM_URL} target="_blank" rel="noreferrer"><img src="/icons/instagram.png" alt="Instagram" /></a><a className="headSocial" href={TIKTOK_URL} target="_blank" rel="noreferrer"><img src="/icons/tiktok.png" alt="TikTok" /></a><a className="headSocial" href={LINE_URL} target="_blank" rel="noreferrer"><img src="/icons/line.png" alt="LINE" /></a><a className="phonePill" href={`tel:${PHONE}`}>{PHONE}</a></div></div></header>}
